(() => {
  ...
  const render = () => {
    ...
    if (state.done) return renderResults(app);
    if (state.phase === 'choose') return renderChoice(app);
    if (state.phase === 'risks') return renderRisk(app);
    const q = state.questions[state.index];
    const dim = state.profile.dimensions.find(d => d[0] === q.dimension);
    app.innerHTML = `<div class="avm-meta"><span>Question ${state.index+1} of ${state.questions.length}</span><span>${Math.round(state.index/state.questions.length*100)}% complete</span></div><div class="avm-progress"><div style="width:${state.index/state.questions.length*100}%"></div></div><div class="avm-eyebrow">${dim[1]}</div><h2 class="avm-question">${q.text}</h2><div class="avm-options">${labels.map((l,i)=>`<button class="avm-option" data-value="${i}"><strong>${l}</strong><span>Score ${i} of 4</span></button>`).join('')}</div><div class="avm-actions"><button class="avm-btn secondary" id="avm-back" ${state.index===0?'disabled':''}>Back</button></div>`;
  ...
  const renderChoice = (app) => { app.innerHTML = `<div class="avm-eyebrow">Choose your assessment</div><h2 class="avm-question">Who are you assessing?</h2><p class="avm-lede">Select the option that best matches your situation. The questions and recommendations will be tailored to your choice.</p><div class="avm-options"><button class="avm-option" data-profile="individual"><strong>Individual AI User</strong><span>Assess your own AI skills, habits, value, and responsible use.</span></button><button class="avm-option" data-profile="company"><strong>Company / Team</strong><span>Assess a team, business unit, or organization’s AI adoption, value, and governance.</span></button></div>`; app.querySelectorAll('[data-profile]').forEach(b=>b.addEventListener('click',()=>{state.profile=state.data.profiles[b.dataset.profile];state.questions=state.profile.questions;state.risks=state.profile.criticalRisks;state.answers=[];state.riskAnswers=[];state.index=0;state.riskIndex=0;state.phase='questions';state.done=false;render();})); };
  const renderRisk = (app) => { const r=state.risks[state.riskIndex]; app.innerHTML=`<div class="avm-meta"><span>Safety check ${state.riskIndex+1} of ${state.risks.length}</span><span>Critical risk review</span></div><div class="avm-eyebrow">One final safety check</div><h2 class="avm-question">${r.text}</h2><p class="avm-lede">Choose Yes only if this is currently true.</p><div class="avm-options"><button class="avm-option" data-risk="1"><strong>Yes</strong><span>This is currently true</span></button><button class="avm-option" data-risk="0"><strong>No</strong><span>This is not currently true</span></button></div>`; app.querySelectorAll('[data-risk]').forEach(b=>b.addEventListener('click',()=>{state.riskAnswers[state.riskIndex]=Number(b.dataset.risk);state.riskIndex++;if(state.riskIndex>=state.risks.length){state.done=true;state.phase='results';}render();})); };
